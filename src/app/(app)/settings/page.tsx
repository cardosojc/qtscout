...
import { DOCUMENT_TYPE_LABELS, type DocumentType, type DocumentSettings } from '@/types/document'
    ...
    <main id="main-content" className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mb-8">
          Configure o número inicial de cada tipo de documento. Esta alteração apenas afeta futuras sequências (novo ano ou tipo ainda sem documentos).
        </p>

        {loading ? (
          <div className="space-y-4">
            {ALL_TYPES.map((t) => (
              <div key={t} className="h-14 bg-gray-100 dark:bg-gray-700 rounded-lg animate-pulse" />
            ))}
          </div>
        ) : (
          <div className="space-y-4">
              ...
              <div key={s.type} className="flex items-center justify-between p-4 border border-gray-200 dark:border-gray-700 rounded-lg">
                <div>
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-400">
                    {s.type === 'ORDEM_SERVICO' ? 'Sequência contínua global' : 'Reinicia a cada ano'}
                  </p>
                </div>
                <div className="flex items-center gap-2">
                  <label className="text-sm text-gray-600 dark:text-gray-300">Número inicial:</label>
                  <input
                    type="number"
                    min={1}
