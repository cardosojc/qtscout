...
export default function OrdemServicoPage() {
  ...
  const fetchItems = useCallback(async () => {
    ...
    setLoading(true)
    try {
      const res = await fetch(`/api/ordem-items?${params.toString()}`)
      if (res.ok) {
  ...
  const handleCreate = async (payload: {
    ...
    const res = await fetch('/api/ordem-items', {
      method: 'POST',
  ...
  const handleDelete = async (item: ItemDTO) => {
    ...
    if (!confirmed) return
    const res = await fetch(`/api/ordem-items/${item.id}`, { method: 'DELETE' })
    if (res.ok) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch('/api/ordens-servico/generate', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (res.ok) {
        ...
        showToast(`Ordem de Serviço ${doc.identifier} criada (${doc.itemCount} itens)`, 'success')
        setGenerating(false)
