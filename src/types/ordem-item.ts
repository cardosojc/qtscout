...
/**
 * Catalog of all item categories. This is the single source of truth for
 * shape (form rendering), scope (permission checks), and routing into the
 * generated OrdemServicoData snapshot.
 *
 * Cross-reference — category → OrdemServicoData bucket (see ordem-assembler.ts):
 *
 *   RESOLUCAO              → determinacoes.resolucoes[]
 *   DETERMINACAO           → determinacoes.determinacoes[]
 *   ATIVIDADE              → atividades.agrupamento[]            (section null)
 *                          → atividades[section][]                (section set)
 *   CRIACAO                → criacaoExtincao.criacao[section][]
 *   EXTINCAO               → criacaoExtincao.extincao[section][]
 *   NOMEACAO_DIRIGENTE     → nomeacoes.dirigentes[]               (PROFILE_REF)
 *   NOMEACAO_SECCAO        → nomeacoes[section][]                 (SCOUT_OR_PROFILE_REF)
 *   NOMEACAO_DEPARTAMENTO  → nomeacoes.departamentos[]
 *   READMISSAO             → efetivo.readmissao[section][]
 *   TRANSFERENCIA          → efetivo.transferencia[section][]
 *   PASSAGEM               → efetivo.passagens[section][]
 *   INVESTIDURA            → efetivo.investiduras[section][]
 *   SAIDA_ATIVO_SECCAO     → efetivo.saidaAtivo[section][]
 *   SAIDA_ATIVO_DIRIGENTE  → efetivo.saidaAtivo.dirigentes[]
 *   PROGRESSO              → sistemaProgresso[section][]
 *   NOITES_CAMPO           → noitesCampo[section][]               (NOITES_REF)
 *   ACCAO_DISCIPLINAR      → justicaDisciplina.accoesDisicplinares[]
 *   DISTINCAO_PREMIO       → justicaDisciplina.distincoesPremios  (joined with "\n\n")
 *   RETIFICACAO            → retificacoes[]
 *
 * Also folded in at generation time (not a manual category):
 *   Scout.joinedAt in OS period → efetivo.admissao[section][]
 *
 * When adding a category:
 *   1. Append an entry here and pick a shape.
 *   2. Extend validateItemData() below if the shape is new.
 *   3. Add a `case` in ordem-assembler.ts mapping it to a snapshot bucket
 *      (and grow OrdemServicoData / the OS view + PDF if you need a new
 *      bucket).
 *   4. The form (item-form.tsx) renders from `shape` automatically — no
 *      change unless you introduce a new shape.
 */
