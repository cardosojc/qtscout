...
/**
 * Catalog of all item categories. Single source of truth for shape (form
 * rendering), scope (permission checks), and routing into the generated
 * OrdemServicoData snapshot.
 *
 * Cross-reference (category → bucket): see `docs/ordem-categories.md`,
 * regenerated by `npm run docs:sync` from this file and `ordem-assembler.ts`.
 *
 ...
 *      bucket).
 *   4. Run `npm run docs:sync` to refresh the catalog doc.
 *   5. The form (item-form.tsx) renders from `shape` automatically — no
