...
(`api/`). The Next.js web app (`apps/web`) is unchanged — it talks to the API
over `fetch` + Bearer token. See `api/README.md` and `api/DEPLOY.md`.
...
└── web/                          # Next.js 15 UI (no business logic / DB)
    ...
    ├── src/lib/api-hooks.ts      # SWR hooks over apiFetch
    ├── src/lib/api-types.ts      # generated from openapi/openapi.json (gen:api-types)
    ├── src/lib/api-schemas.ts    # friendly aliases for the generated types
    ├── src/lib/supabase/         # client.ts, server.ts, middleware.ts (session refresh)
    └── src/flags.ts              # Vercel flags (UI visibility + /documents gating)

api/                              # FastAPI service — the backend (Docker on Railway)
├── pyproject.toml  uv.lock  Dockerfile
├── app/
│   ├── main.py                   # app, CORS, router includes, {"error":…} handler
│   ├── config.py                 # pydantic-settings (env)
│   ├── db.py                     # async SQLAlchemy engine/session (pooler-safe)
│   ├── deps.py                   # CurrentUser / AdminUser dependencies
│   ├── auth.py supabase_admin.py # Bearer verify; GoTrue admin (create/delete user)
│   ├── routers/                  # one module per domain (13)
│   ├── models/                   # SQLAlchemy 2.0 models (11 tables)
│   ├── schemas/                  # Pydantic v2 request/response (drives OpenAPI)
│   ├── core/                     # ordem_*, siie_*, document_utils, scout_utils,
│   │                             #   leader_roles, ordem_categories(.json)
...
packages/
├── types/   # @qtscout/types — TS types + FE runtime helpers (labels, validators,
│            #   scoutDisplayName). ordem-categories.generated.ts is generated from
│            #   api/app/core/ordem_categories.json (npm run sync:categories).
└── core/    # @qtscout/core — only ano-escutista.ts (date logic the web imports)
...
`transpilePackages`. The web depends only on `@qtscout/types` (+
`@qtscout/core/ano-escutista`) and the generated `api-types`/`api-schemas`.

## Domain model

Unchanged by the migration — same 11 tables (now SQLAlchemy models in
`api/app/models/`, mapping the existing camelCase Prisma columns).
...
3. **API auth** (`api/app/deps.py`, `auth.py`): `current_user` extracts the
...
`api/app/supabase_admin.py` (GoTrue REST, `SUPABASE_SECRET_KEY`). Two endpoints
...
User picks category (filtered by permissions)
  ...
  └─> POST /api/ordem-items   (api/app/routers/ordem_items.py)
...
The catalog is **single-sourced** from `api/app/core/ordem_categories.json`: the
...
- Models in `api/app/models/` map the existing tables — snake_case Python attrs
  to **exact camelCase columns** (`mapped_column("createdAt", …)`); PG enums bound
  with `create_type=False`; cuid/uuid id factories.
- `api/app/db.py` normalises `DATABASE_URL` for asyncpg and disables the
...
- Response parity: `api/app/schemas/base.py` — `ORMModel` emits camelCase keys;
...
- **Web** → Vercel project `qtscout` (Next.js). Env: `NEXT_PUBLIC_*` +
  ...
  server side needs.
- **API** → Docker container on **Railway** (`api/Dockerfile`; `render.yaml` is a
  ...
  `WEB_ORIGIN` + the `Authorization` header. Full cutover steps: `api/DEPLOY.md`.

## Commands

- **API** (from `api/`, `uv`): `uv run uvicorn app.main:app --reload --port 3001`,
