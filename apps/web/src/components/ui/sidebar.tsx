...
const membrosNavItem = {
  href: '/membros',
  label: 'Membros',
  badge: undefined,
  icon: (
    <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 20h5v-2a4 4 0 00-3-3.87M9 20H4v-2a4 4 0 013-3.87m3-2.13a4 4 0 110-8 4 4 0 010 8zm6 0a3 3 0 100-6 3 3 0 000 6z" />
    </svg>
  ),
  children: undefined,
}
...
function buildDocumentsNavItem(enabledDocTypes: EnabledDocTypes) {
  ...
  // OS is generated from items on its own page, so skip it when picking the
  // default target for the "Novo Documento" shortcut.
  const firstEnabledForNew = oficio ? 'OFICIO' : circular ? 'CIRCULAR' : null
  ...
  if (firstEnabledForNew) {
    children.push({
      href: `/documents/new?type=${firstEnabledForNew}`,
...
export function Sidebar({ enabledDocTypes }: SidebarProps) {
  ...
  const settingsNavItem = {
    ...
    children: [
      { href: '/settings', label: 'Numeração', icon: null as React.ReactNode, badge: undefined },
      { href: '/settings/users', label: 'Utilizadores', icon: null as React.ReactNode, badge: undefined },
    ],
  }
  ...
  const navItems = [
    ...
    ...(ordem ? [ordemServicoNavItem] : []),
    membrosNavItem,
    searchNavItem,
    ...(user?.role === 'ADMIN' ? [settingsNavItem] : []),
  ]
