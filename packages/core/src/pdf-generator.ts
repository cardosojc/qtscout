...
import type { OrdemServicoData, OSAtividade, OSNomeacao, OSNoitesMilestone } from '@qtscout/types/ordem-servico'
...
function loadImageBase64(relativePath: string): string {
  // Assets ship inside this package (packages/core/assets). Resolve relative to
  // the module first, then fall back to an app's public/ dir (cwd) for safety.
  const candidates: string[] = []
  try {
    const here = path.dirname(fileURLToPath(import.meta.url))
    candidates.push(path.join(here, '..', 'assets', relativePath))
  } catch {
    // import.meta may be unavailable under some bundlers; rely on cwd fallbacks.
  }
  ...
  console.error(`Error loading image ${relativePath}: not found in ${candidates.join(', ')}`)
  return ''
}
